...
const Footer = () => {
  const { businessName, brandTag, tagline, address, phone, instagram, hours } = seoConfig;
    ...
    <footer className="bg-secondary text-secondary-foreground">
      <div className="max-w-7xl mx-auto section-padding py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="md:col-span-1">
            <h3 className="font-heading text-xl font-bold tracking-tight mb-4">
              {brandTag.split('.')[0]}<span className="text-primary">.</span>{brandTag.split('.')[1]}
            </h3>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 text-sm text-secondary-foreground/60">
              <span className="flex items-center gap-2"><MapPin size={14} className="text-primary" /> {address.short}</span>
              <span className="flex items-center gap-2"><Phone size={14} className="text-primary" /> {phone}</span>
              <a href={instagram.url} target="_blank" rel="noopener" className="flex items-center gap-2 hover:text-primary transition-colors">
                <Instagram size={14} className="text-primary" /> {instagram.handle}
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 text-sm text-secondary-foreground/60">
              <span className="flex items-center gap-2"><Clock size={14} className="text-primary" /> {hours.weekday}</span>
              <span className="ml-[22px]">{hours.saturday}</span>
              <span className="ml-[22px]">{hours.sunday}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-secondary-foreground/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs text-secondary-foreground/40">© {new Date().getFullYear()} {businessName}. All rights reserved.</p>
